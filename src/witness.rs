...
pub struct WitnessEnv<F> where F: Field {
...
impl<F: Field> CompiledCircuit<F> {
    ...
    pub fn compute_var(&self, env: &mut WitnessEnv<F>, var: CellVar) -> Result<F> {
        // fetch cache first
        ...
        match &self.circuit.witness_vars[&var.index] {
            ...
            Value::Inverse(v) => {
                ...
                let res = v.inverse().unwrap_or_else(F::zero);
                env.cached_values.insert(var, res); // cache
                Ok(res)
            }
            ...
            if is_not_public_input {
                ...
                match gate.typ {
                    ...
                    crate::circuit_writer::GateKind::DoubleGeneric => {
                        let c = |i| gate.coeffs.get(i).copied().unwrap_or_else(F::zero);
                        let w = &witness_row;
                        let sum1 =
                            c(0) * w[0] + c(1) * w[1] + c(2) * w[2] + c(3) * w[0] * w[1] + c(4);
                        let sum2 =
                            c(5) * w[3] + c(6) * w[4] + c(7) * w[5] + c(8) * w[3] * w[4] + c(9);
                        if sum1 != F::zero() || sum2 != F::zero() {
                            return Err(Error::new(
                                "runtime",
